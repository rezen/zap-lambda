    ...
    for key, alert_list in sorted(alert_dict.items()):
        ...
        if inc_rule(config_dict, key, inc_extra):
            ...
            if min_level <= zap_conf_lvls.index(level):
                print_rule(
                    level, alert_list, detailed_output, user_msg, in_progress_issues
                )
...
def start_zap(port, extra_zap_params):
    ...
    env[
        "JAVA_OPTS"
    ] = "-Djava.util.prefs.systemRoot=/tmp -Djava.util.prefs.userRoot=/tmp " + env.get(
        "JAVA_OPTS", ""
    )
...
def zap_ajax_spider(zap, target, max_time):
    ...
    while zap.ajaxSpider.status == "running":
        logging.debug(
            "Ajax Spider running, found urls: " + zap.ajaxSpider.number_of_results
        )
        time.sleep(5)
